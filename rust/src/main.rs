...
fn main() {
    ...
    let _arduino_thread_handle = thread::spawn(|| arduino_communication_handler(rx_to_arduino, tx_to_simconnect));

    // Start thread to handle communication with MSFS via simconnect
    let _simconnect_thread_handle = thread::spawn(|| simconnect_handler(rx_to_simconnect, tx_to_arduino));
